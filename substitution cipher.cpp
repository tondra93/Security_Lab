...
int main()
{
    ...
    for(i=0;i<26;i++)
    {
        ...
        per=(x/y)*100.0;
        char_frquency[i].ch=ch;
        char_frquency[i].cnt=frequency[i];
        char_frquency[i].per=per;
    }
    cout<< "--------- " << endl;
    sort(char_frquency,char_frquency+26,cmp);
    for(i=0;i<26;i++)
    {
        ch=char_frquency[i].ch;
        freq=char_frquency[i].cnt;
        per=char_frquency[i].per;
        cout<< ch << "    " << freq << "    " << per << endl;
    }
    cout<< "\n" << cipher << endl;
	message = cipher;
